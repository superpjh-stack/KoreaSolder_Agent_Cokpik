...
from goryeo_solder_agent.voice import VoiceService
from openai import OpenAI

from goryeo_solder_agent import (
    DEFAULT_MODEL, MAX_RETRIES, REQUEST_TIMEOUT_SECONDS, QUESTION_GROUPS, WELCOME_MESSAGE,
    create_repository, GoryeoSolderToolRegistry,
    ManufacturingAgent, timestamp, user_question_history, work_order_snapshot, risk_label,
)
...
st.set_page_config(page_title="고려솔더 Agent Cockpit", page_icon="⚙️", layout="wide", initial_sidebar_state="collapsed")
st.markdown(f"<style>{(BASE_DIR / 'assets' / 'cockpit.css').read_text()}</style>", unsafe_allow_html=True)

repository = create_repository(os.getenv("SQLITE_PATH", str(BASE_DIR / "data" / "goryeo_solder_demo.db")))
registry = GoryeoSolderToolRegistry(repository)
...
with st.sidebar:
    ...
    st.divider()
    st.subheader("지식문서")
    uploads = st.file_uploader("합금·용해·검사·출하 SOP 문서", accept_multiple_files=True, type=["pdf", "docx", "txt", "md", "csv"])
    ...
    if st.button("문서 인덱싱", use_container_width=True, disabled=not uploads or not upload_agent):
        try:
            ...
            st.error("문서 인덱싱에 실패했습니다. API 연결과 파일 형식을 확인해 주세요.")
    st.caption(f"저장된 지식문서: {len(repository.knowledge_documents())}개 (로컬 샘플 포함)")
    st.divider()
    st.caption("모든 초기 데이터는 가상 데모입니다. 생산중지·원료대체·배합변경·품질·출하는 담당자가 승인합니다.")
...
<header class="factory-header">
  <div><div class="brand-line">GORYEO SOLDER / MANUFACTURING</div>
  <h1>고려솔더 AI Agent</h1>
  <p>원료 LOT부터 용해·주조·압연·포장·출하까지 근거를 연결합니다.</p></div>
  <div class="header-meta"><strong>생산관리 워크스페이스</strong><span>검증용 데모 데이터</span></div>
</header>
<div class="process-strip"><b>제조 공정</b><span>원료 입고·검사</span><i>→</i><span>배합·용해</span><i>→</i><span>주조</span><i>→</i><span>압연·권취</span><i>→</i><span>포장·출하</span></div>
<div class="overview-label"><strong>현장 주요 지표</strong><span>작업지시 / 예지보전 / 품질 · 2026-09 샘플</span></div>
""", unsafe_allow_html=True)

with st.container(key="kpis"):
    metrics = st.columns(5)
    metrics[0].metric("진행 작업", f"{kpi['active_orders']}건")
    metrics[1].metric("용해 주의", f"{kpi['melt_alerts']}건")
    metrics[2].metric("입고 보류", f"{kpi['incoming_holds']}건")
    metrics[3].metric("품질 확인", f"{kpi['quality_holds']}건")
...
with context_col:
    ...
    with st.container(border=True, key="context"):
        ...
        with st.expander("작업지시 상황·공정 확인"):
            lot_id = st.selectbox("조회 작업지시", [row["work_order"] for row in repository.get_work_orders()])
            snap = work_order_snapshot(repository, lot_id)
            label, icon = risk_label(snap)
            st.write(f"{icon} {label} · {snap['job'].get('product_name', '')}")
            st.caption(f"작업 {lot_id} · 데모 진척 {snap['job'].get('progress_pct', 0):.0f}%")
            for heading, rows in [("주조·압연", snap["processes"]), ("용해 측정", snap["press"]), ("용해 배치", snap["die"]), ("주의 측정", snap["risks"]), ("원료·수입검사", snap["incoming"]), ("품질검사", snap["outgoing"]), ("출하 승인", snap["shipments"])]:
                st.markdown(f"**{heading}**")
                if rows:
                    st.dataframe(rows, hide_index=True)
                else:
                    st.caption("미확인 · 연결 기록이 없습니다.")
            st.caption("작업지시에 원료 LOT·용해 배치·주조·압연·품질·포장·출하 이력을 연결합니다. 모든 수치는 가상 데모입니다.")
            if st.button("이 작업지시 브리핑", key="lot_brief", use_container_width=True):
                st.session_state.pending_question = f"{lot_id}의 원료·용해·주조·압연·검사·출하 확인사항을 브리핑해줘"
                st.rerun()

        st.markdown("##### RAG 지식문서")
        st.caption("저장된 샘플·업로드 문서 / 세션 종료 후에도 유지")
        if documents:
            query = st.text_input("문서 검색", placeholder="예: 용해, 조성, 수입검사", key="document_query")
...
with chat_col:
    ...
    with st.container(border=True, height=580, key="chat_panel"):
        st.markdown('''<div class="chat-shell-header">
          ...
          <span>작업지시·원료·용해·품질·출하를 한 곳에서 확인합니다.</span></div>
          <div class="chat-shell-status"><i></i> 데모 Data Hub</div>
        </div>''', unsafe_allow_html=True)
        if not user_question_history(st.session_state.messages):
            st.markdown('''<div class="chat-intro"><h2>소재 입고부터 출하까지,<br>현장의 판단을 빠르게.</h2>
            <p>원료 보류, 용해 편차, 검사·출하 상태를 질문하세요.<br>LOT 데이터와 기준서를 연결해 확인합니다.</p></div>''', unsafe_allow_html=True)
        if not agent:
    ...
    typed_question = st.chat_input("원료 LOT·용해 배치·작업지시·검사·출하에 질문하세요", disabled=agent is None)
...
st.markdown('<footer class="workspace-footer">고려솔더 제조 업무 지원 | 표시된 수치와 예측은 검증용 가상 데이터입니다.</footer>', unsafe_allow_html=True)
